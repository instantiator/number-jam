/**
 * Integration test for the fast-alpr Python daemon detection engine.
 *
 * Starts the Python daemon, sends two real plate images, and verifies the
 * shape of the response.
 *
 * Skipped unless RUN_INTEGRATION_TESTS=1 is set.
 * Also skipped gracefully when the fast_alpr Python package is not installed
 * (engine.check() fails) so that the docker-only CI path is not broken.
 *
 * Fixtures:
...
describe("fast-alpr engine integration", () => {
  let engine: FastAlprEngine;
  /** Set to true when fast_alpr is unavailable; individual tests return early. */
  ...
  beforeAll(async () => {
    ...
    engine = new FastAlprEngine();
    try {
      await engine.check();
      await engine.startup();
    } catch (err) {
      unavailable = true;
      console.warn(`[fast-alpr] Skipping integration tests: ${(err as Error).message}`);
    }
  }, 60_000);

  afterAll(async () => {
    if (SKIP || unavailable) return;
